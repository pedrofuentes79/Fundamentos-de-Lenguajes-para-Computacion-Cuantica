...
El tipo de $\textbf{And}$ es: $\text{Bool} \to \text{Bool} \to \text{Bool}$, donde $\text{Bool} = \text{Unit} + \text{Unit}$.

\begin{center}
\infer[\text{$\to$-intro}]{And := $\lambda x. \lambda y. match(x, w.match(y, a.inl(*), b.inr(*)), z.inr(*)) : \text{Bool} \to \text{Bool} \to \text{Bool}$}{}
\end{center}

\subsection{Tipo de Or}

El tipo de $\textbf{Or}$ es: $\text{Bool} \to \text{Bool} \to \text{Bool}$.

\begin{center}
\infer[\text{$\to$-intro}]{Or : \text{Bool} \to \text{Bool} \to \text{Bool}}{
  \infer[\text{$\to$-intro}]{\lambda y.\ldots : \text{Bool} \to \text{Bool}}{
    \infer[\text{match-elim}]{match(x, a.inr(*), b.\ldots) : \text{Bool}}{
      x : \text{Bool}
    }
  }
}
